...
def _require_minio_credential(env_name: str) -> str:
    """获取 MinIO 凭据；生产环境强制显式配置，开发环境兜底默认值。

    生产环境若未显式配置 MINIO_ACCESS_KEY / MINIO_SECRET_KEY，则拒绝启动，
    避免使用公开默认凭据（minioadmin/minioadmin）暴露对象存储。
    """
    value = os.getenv(env_name, "").strip()
    if value:
        return value
    if _is_production_env():
        raise RuntimeError(f"{env_name} 未配置，请在生产环境的 .env.prod 中设置 MinIO 凭据")
    return "minioadmin"
...
class MinIOClient:
    ...
    def __init__(self):
        ...
        self.access_key = _require_minio_credential("MINIO_ACCESS_KEY")
        self.secret_key = _require_minio_credential("MINIO_SECRET_KEY")
    ...
    def upload_file(
        ...
        try:
            ...
            if bucket_name in self.PUBLIC_READ_BUCKETS:
                ...
                url = f"http://{self.public_endpoint}/{bucket_name}/{quote(object_name, safe='/')}"
