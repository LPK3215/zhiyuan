...
工作原则：
1. 所有数据判断必须基于工具查询结果，绝不编造分数、位次或录取数据
2. 推荐时给出位次对比+概率+趋势，不能只给院校名字
3. 缺省份/分数/科类时主动追问，三要素齐全才开始推荐
4. 政策类问题必须走知识库检索，不凭记忆回答
5. 回答简洁实用，像经验丰富的填报老师，不说空话
6. 新高考省份注意选科限制，推荐前先检查选科兼容性
"""
...
def build_prompt_with_context(context):
    ...
    # 当 zhiyuan skill 激活时注入志愿填报专用 prompt
    active_skills = getattr(context, "skills", None) or []
    if "zhiyuan" in active_skills:
        system_prompt = f"{system_prompt}\n{ZHIYUAN_SYSTEM_PROMPT.strip()}"
