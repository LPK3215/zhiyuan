...
本技能的 `scripts/query.py` 会在应用层对 LLM 生成的 SQL 进行黑名单 + 白名单校验（仅允许 `SELECT/SHOW/DESCRIBE/EXPLAIN`），但应用层校验不能作为唯一防线。**部署时必须为该技能使用的 MySQL 账号授予最小只读权限**，确保即使校验被绕过也无法执行写操作或访问未授权库表。

推荐授权语句（在目标 MySQL 上以管理员执行）：

```sql
-- 创建专用的只读账号（替换 <password> 为强随机密码）
CREATE USER 'yuxi_reporter'@'%' IDENTIFIED BY '<password>';

-- 仅授予目标业务库的 SELECT 权限（替换 <business_db> 为实际库名）
GRANT SELECT ON <business_db>.* TO 'yuxi_reporter'@'%';

-- 显式拒绝写权限（纵深防御，确保 REVOKE 兜底）
REVOKE INSERT, UPDATE, DELETE, DROP, ALTER, CREATE, TRUNCATE, INDEX, GRANT OPTION ON <business_db>.* FROM 'yuxi_reporter'@'%';
...
```
MYSQL_USER=yuxi_reporter
MYSQL_PASSWORD=<password>
MYSQL_DATABASE=<business_db>
```

**禁止**使用 `root` 或具备写权限的账号作为本技能的连接账号。
