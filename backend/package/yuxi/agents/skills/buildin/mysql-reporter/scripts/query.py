...
class MySQLSecurityChecker:
    ...
    # SQL 最大长度（字符），防止 LLM 生成超长查询导致资源耗用
    MAX_SQL_LENGTH = 8192
...
def run_query(sql: str, timeout: int) -> str:
    ...
    # 审计日志：记录即将执行的 SQL 与超时，便于事后追溯。
    # 注意：该 Skill 的 MySQL 账号应仅授予 SELECT 只读权限（DB 层兜底），
    # 不依赖应用层黑名单作为唯一防线。
    audit_msg = f"[mysql-reporter audit] executing SQL (timeout={timeout or 60}s): {sql[:500]}"
    if len(sql) > 500:
        audit_msg += f"... ({len(sql)} chars total)"
