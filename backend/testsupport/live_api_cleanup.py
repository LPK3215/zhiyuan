...
async def cleanup_pytest_knowledge_resources(
        ...
        for list_path, id_field, delete_prefix in resource_specs:
            ...
            for resource in resources:
                ...
                if delete_response.status_code not in {200, 404}:
                    failures.append(f"Failed to delete evaluation resource {resource_id}: {response.text}")
    ...
    for database in databases:
        ...
        if delete_response.status_code not in {200, 404}:
            failures.append(f"Failed to delete knowledge database {kb_id}: {response.text}")
